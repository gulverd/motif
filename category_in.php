...
<html lang="en">
  ...
  <body class="product-page single-product group-product">
            ...
            <div class="woocommerce product-listing columns-4 clearfix">
              <ul class="products-tabs nav nav-pills">
                <?php
                  ...
                  while($row1 = mysqli_fetch_array($run1)){
                    $sub_id     = $row1['id'];
                    $sub_title  = $row1['title_en'];  
                    echo '<li><a href="sub_category_in.php?cat_id='.$cat_id.'&sub_id='.$sub_id.'">'.$sub_title.'</a></li>';
                  }
                ?>
              </ul>
              <div class="desc-review-content tab-content clearfix">
                <div id="1a" class="tab-pane active">
                  <?php 
                    $query2 = "SELECT a.id as id, a.pic_url as pic_url , a.title_en as title_en , b.title_en as designer, c.id as brand_id ,c.title_en as brand_name FROM products a JOIN designers b ON  a.designer_id = b.id  JOIN brands c ON a.brand_id = c.id WHERE a.cat_id = '$cat_id'";
